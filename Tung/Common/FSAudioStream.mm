...
//#import <AudioToolbox/AudioToolbox.h> // added to header
...
- (id)init
{
    ...
    if (self) {
        ...
        self.maxBounceCount    = 2;   // Max number of bufferings in bounceInterval seconds
...
void AudioStreamStateObserver::samplesAvailable(AudioBufferList samples, AudioStreamPacketDescription description)
{
    ...
    */
    // Jamie added: delegate method more compatible with ExtAudioFileWrite - passes AudioBufferList directly
    if ([priv.delegate respondsToSelector:@selector(audioStream:audioBufferList:count:)]) {

        NSUInteger count = description.mDataByteSize / sizeof(int16_t);
        [priv.delegate audioStream:priv.stream audioBufferList:samples count:count];
    }
