...
- (IBAction)updateProfileData:(id)sender {
    ...
    [tungCommonObjects saveUserData:_profileData];
    ...
    [_tungObjects updateUserWithDictionary:updates withCallback:^(NSDictionary *jsonData) {
        if ([[jsonData objectForKey:@"nModified"] intValue] > 0) {
            NSLog(@"success updating user");
            self.navigationItem.title = @"Saved";
            [NSTimer scheduledTimerWithTimeInterval:0.5 target:self selector:@selector(cancelEditProfile:) userInfo:nil repeats:NO];
        }
